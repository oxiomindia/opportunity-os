...
export default async function DashboardPage() {
  ...
  const pendingApproval = bills.filter((bill) => bill.status === 'pending-approval').length;
  const awaitingPaymentStatuses = new Set(['approved', 'payment-scheduled', 'partially-paid']);
  const awaitingPayment = bills.filter((bill) => awaitingPaymentStatuses.has(bill.status)).length;
  const overdueBills = bills.filter((bill) => awaitingPaymentStatuses.has(bill.status) && bill.dueDate < today).length;

  return <div className="flex flex-col gap-5">
    <section className="rounded-xl border border-slate-200 bg-white p-5"><h1 className="text-2xl font-semibold text-slate-950">Billing overview</h1><p className="mt-2 text-sm text-slate-600">Live metrics from your organization&apos;s invoices and bills.</p></section>
    {showAr && (
      <section>
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-slate-500">Accounts receivable</h2>
    ...
    {showAp && (
      <section>
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-slate-500">Accounts payable</h2>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {[['Total bills', bills.length], ['Pending approval', pendingApproval], ['Awaiting payment', awaitingPayment], ['Overdue', overdueBills]].map(([label, value]) => <article key={label} className="rounded-xl border border-slate-200 bg-white p-5"><p className="text-sm font-medium text-slate-500">{label}</p><p className="mt-2 text-3xl font-semibold text-slate-950">{value}</p></article>)}
        </div>
      </section>
    )}
    <section className="grid gap-5 lg:grid-cols-[1fr_20rem]">
      ...
      <article className="rounded-xl border border-slate-200 bg-white p-5">
        ...
        <div className="mt-5 flex flex-col gap-3">
          {showAr && <Link href="/invoices/new" className="rounded-lg bg-blue-600 px-4 py-2.5 text-center text-sm font-semibold text-white hover:bg-blue-700">New invoice</Link>}
          {showAr && <Link href="/customers" className="rounded-lg border border-slate-200 px-4 py-2.5 text-center text-sm font-semibold text-slate-700 hover:border-blue-200 hover:text-blue-700">Manage customers</Link>}
          {showAp && <Link href="/bills/new" className="rounded-lg bg-amber-600 px-4 py-2.5 text-center text-sm font-semibold text-white hover:bg-amber-700">New bill</Link>}
          {showAp && <Link href="/vendors" className="rounded-lg border border-slate-200 px-4 py-2.5 text-center text-sm font-semibold text-slate-700 hover:border-blue-200 hover:text-blue-700">Manage vendors</Link>}
        </div>
      </article>
    </section>
    {showAr && (
