...
const categoryAccents: Record<string, { bg: string; text: string; ring: string }> = {
  ...
  'compliance-automation': { bg: 'bg-emerald-50', text: 'text-emerald-700', ring: 'ring-emerald-100' },
};
  ...
  return (
    <article className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-7 shadow-sm ring-1 ring-transparent transition-all hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md">
      <div className="flex items-start justify-between gap-3">
        ...
        {badge && (
          <span className={`rounded-full px-3 py-1 text-xs font-semibold ${isComingSoon ? 'bg-slate-900 text-white' : `${accent.bg} ${accent.text}`}`}>
            {badge}
          </span>
        )}
      </div>

      <h3 className="mt-5 text-xl font-semibold tracking-tight text-slate-950">{getProductDisplayName(product)}</h3>
      ...
      <div className="mt-7 flex flex-col gap-2.5 sm:flex-row">
        ...
        >
          {isComingSoon ? 'Get Early Access' : 'Book Demo'}
        </Link>
      </div>
    </article>
  );
