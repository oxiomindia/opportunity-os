    ...
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-[15px] font-semibold text-slate-900">Opportunity Results</h2>
          {results.length > 0 && (
      ...
      {results.length === 0 ? (
        <div className="py-16 text-center">
          ...
          <p className="text-[13px] text-slate-400 mt-1">Configure your research and click "Run Research" to discover opportunities.</p>
        </div>
      ) : (
          ...
          <table className="w-full min-w-[700px]">
            <thead>
              <tr className="border-b border-slate-200">
                {['Source', 'Title', 'Summary', 'Severity', 'Score', ''].map((h, i) => (
                  <th key={i} className="px-4 py-3 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {sorted.map((row, idx) => (
                <tr
                  key={row.id}
                  className={`border-b border-slate-50 transition-colors hover:bg-slate-50 ${idx % 2 === 1 ? 'bg-slate-50/50' : ''}`}
                >
                  ...
                  <td className="px-4 py-3.5">
                    <span className="text-[13px] font-semibold text-slate-900">{row.title}</span>
                  </td>
                  <td className="px-4 py-3.5 max-w-[300px]">
                    <span className="text-[13px] text-slate-500 leading-relaxed line-clamp-2">{row.summary}</span>
                  </td>
                  <td className="px-4 py-3.5">
