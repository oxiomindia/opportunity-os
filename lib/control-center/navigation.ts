...
export const controlCenterModules: ControlCenterModule[] = [
  ...
  // milestone), and Universal Report Platform (URP milestone) are read-only
  // Admin Console views onto lib/engine/, lib/events/, and lib/urp/
  // respectively. The remaining five stay reserved -- these still render as
  // disabled/"Planned" rows (see ControlCenterShell) so the information
  // architecture is right before the Webhook Engine/etc. exist. No page or
