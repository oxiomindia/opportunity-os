...
|---|---:|---|---|---|
| 2026-07-27 | TBD | `TBD` | Pre-push authentication smoke repair | Rebased onto PR #26, made local demo invalid-credential handling deterministic, and separated demo feedback validation from live durable-feedback E2E coverage. |
| 2026-07-27 | TBD | `TBD` | Organization onboarding repair | Made organization creation repair a missing profile atomically, assign Owner membership, write audit evidence, and log complete Supabase/Postgres diagnostics. |
| 2026-07-27 | TBD | `TBD` | Supabase Auth endpoint diagnostics | Validated the configured Supabase URL, rejected application-domain endpoints, guarded Auth JSON parsing, and added bounded response diagnostics/tests. |
...
| AUTH-002 | Signup with validated identity metadata | `app/signup/**`, `lib/auth/{signup,signup-error}.ts`, `lib/supabase/{config,auth-fetch,server}.ts` | `auth.users`, `profiles` | Signup Server Action, Supabase `signUp` | `/signup` | `signup.test.ts`, `signup-error.test.ts` | — | Signup navigation/layout only | 🟠 Development |
...
| TEN-001 | First user creates organization and Owner membership | `app/onboarding/**`, migrations 0001/0006/0007/0008 | `profiles`, `organizations`, `organization_members`, `audit_logs`; bootstrap trigger/RPCs | Organization action and onboarding RPCs | `/onboarding`; signup/callback flow | — | Conditional RLS test does not validate bootstrap | Conditional organization onboarding and profile-repair integration | 🟠 Development |
...
| QA-001 | Automated unit/integration/E2E release gate | `package.json`, `playwright.config.ts`, `.github/workflows/ci.yml`, `tests/**` | Test-only Supabase dependencies | Test commands | Test reports | 28 active tests | 2 conditional Supabase tests | Chromium-only conditional suite | 🟠 Development |
...
| AUTH-002 | Register a validated account | `SignupForm` | `/signup` | `signup` | — | auth.users, profiles and bootstrap targets | `bootstrap_new_auth_user`; `on_auth_user_created` | Profile/tenant policies apply after creation | Supabase variables, `NEXT_PUBLIC_APP_URL`; Supabase Auth/email | Signup/error/config/Auth-fetch unit; layout-only E2E |
...
| TEN-001 | Bootstrap first organization with Owner | `OnboardingForm` | `/onboarding`; signup/callback | `createOrganization` | Callback participates | profiles, organizations, organization_members, audit_logs | `create_organization`, `complete_signup_onboarding`, `bootstrap_new_auth_user`; `on_auth_user_created` | Profile, organization, membership, audit policies | Supabase variables, `DATABASE_URL`; Supabase | Conditional manual-onboarding E2E and live profile-repair/RPC integration; bootstrap trigger still unverified |
...
|---|---|---|---|---|---|
| TD-01 | Production migrations 0006/0007/0008 and trigger/RPC state are not verifiable from source alone. | First signup can fail atomically. | Add deploy migration gate and production catalog smoke. | M | **Critical** |
...
| 0008 | Repair missing profile during manual organization onboarding | ✅ | Replaces `create_organization`; production application unverified. |
...
|---|---|---|---|---|---|---|---|
| RISK-001 | First-user signup fails if the Auth endpoint is misconfigured or migrations/trigger/RPC are absent or invalid. | High | High | Critical | Deployment migration gate, catalog assertions and disposable email-to-dashboard test. | Engineering / Release | Mitigated in code with endpoint validation/diagnostics; production unverified |
...
|---|---|---|---|---|---|---|
| INC-2026-001 | 2026-07-27 | First-administrator signup returned a generic failure; the signup action discarded provider diagnostics, and first-user bootstrap migration/journal/deployment state was inconsistent. | Repository baseline `17b0197` adds error mapping/logging and migration 0007 bootstrap; subsequent fixes add endpoint diagnostics and migration 0008, which repairs a missing profile inside atomic organization creation. Production application and end-to-end resolution are **not verified**. | Migration deployment gate, catalog checks, full first-user integration test, production smoke and monitoring. | TBD / not evidenced in local history | Unreleased / not verified |
...
| Unit tests | 30 discovered: 28 passed locally, 2 conditional Supabase integration tests skipped | Pass with conditional gap | All deterministic units required |
...
|---|---|---|---|
| Newly completed features | Organization onboarding profile repair and complete RPC error diagnostics completed in repository code; Supabase Auth endpoint validation remains present. | Config/fetch unit tests and signup client wiring. | Do not advance lifecycle without implementation/test evidence. |
| Regressions | Production signup HTML/JSON parse failure reported; root response is now captured before parsing, but production resolution is unverified. | Incident INC-2026-001 and Auth diagnostic tests; no production access in this revision. | Add automated documentation/traceability checks where practical. |
| Newly introduced technical debt | No known runtime debt introduced; production endpoint verification remains an operational requirement. | Documentation-only diff. | Assign audit owners and enforce update policy in PR review. |
...
| Test coverage gaps | First-user trigger, verification/recovery, full RLS roles, uploads, browsers/devices, a11y, performance and production smoke remain open; manual onboarding now has a conditional live repair test. | QA matrix and Code Quality Dashboard. | Execute roadmap Next Sprint testing items first. |
...
1. Prove migrations 0006/0007/0008, trigger/RPC state, email delivery and first-user Owner bootstrap in a production-like disposable test.
...
1. Verify and integration-test migrations 0006/0007/0008, trigger/RPC and first-user email-to-dashboard flow.
...
- No proof that migrations 0006/0007/0008, `on_auth_user_created`, `bootstrap_new_auth_user`, and `complete_signup_onboarding` are applied/active in the target environment.
